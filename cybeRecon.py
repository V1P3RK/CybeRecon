...
# Menu d'accueil V1P3RK
print("\n")
f = Figlet(font='standard')
print(colored(f.renderText('V1P3RK Pentest'), 'red', attrs=['blink', 'bold']))
print(colored('Script de Pentest codé par :', 'white'), colored('V1P3RK', 'cyan'))
...
if internet_status:
    print(colored("Connexion Internet : OK", 'green'))
else:
    print(colored("Connexion Internet : NON DISPONIBLE", 'red'))

# Vérification de la connexion via Tor
def check_tor():
    try:
        response = requests.get("https://check.torproject.org", timeout=5).text
        if "Congratulations. This browser is configured to use Tor" in response:
            return True
    except requests.ConnectionError:
        pass
    return False
...
if tor_status:
    print(colored("Vous êtes connecté via Tor", 'green'))
else:
    print(colored("Vous n'êtes pas connecté via Tor. Voulez-vous vous connecter ? (y/n)", 'red'))
    choix = input().lower()
    if choix == 'y':
        print(colored("Veuillez démarrer Tor et configurer votre proxy SOCKS5.", 'yellow'))
...
print(colored(f"Utilisation CPU : {cpu_usage}%", 'yellow'))
print(colored(f"Utilisation RAM : {ram_usage}%", 'yellow'))
print(colored(f"Espace disque utilisé : {disk_usage}%", 'yellow'))
...
plt.title('État des ressources de la machine')
plt.show()
...
try:
    ...
    location = requests.get(f"https://ipapi.co/{public_ip}/country_name/").text
except:
    public_ip = "Inconnu"
    location = "Inconnu"
